...
export const AnalyticsView: React.FC<AnalyticsViewProps> = ({ user }) => {
  ...
  // ===== FILTER LOGIC =====
  const filteredAppraisals = mockAppraisals.filter((app) => {
    const userRoleObj = mockUsers.find((u) => u.id === app.employeeId);
    if (!userRoleObj) return false;

    const matchesLocation =
      selectedLocation === "ALL" || userRoleObj.location === selectedLocation;
    const matchesDept =
      selectedDept === "ALL" || userRoleObj.department === selectedDept;

    return (
      app.cycleId === mockActiveCycle?.id && matchesLocation && matchesDept
    );
  });

  // ===== METRICS =====
  const totalInScope = filteredAppraisals.length;
  const completedScenarios = filteredAppraisals.filter(
    (a) => a.currentStage === "Completed",
  ).length;
  const completionRate =
    totalInScope === 0
      ? 0
      : Math.round((completedScenarios / totalInScope) * 100);

  const scoreAppraisals = filteredAppraisals.filter(
    (a) =>
      a.calibration?.calibratedScore || a.managerAppraisal?.calculatedScore,
  );
  const averageRating =
    scoreAppraisals.length === 0
      ? 0
      : parseFloat(
          (
            scoreAppraisals.reduce((acc, current) => {
              const ratingVal =
                current.calibration?.calibratedScore ||
                current.managerAppraisal?.calculatedScore ||
                0;
              return acc + ratingVal;
            }, 0) / scoreAppraisals.length
          ).toFixed(2),
        );

  const calibratedScenarios = filteredAppraisals.filter(
    (a) =>
      a.calibration?.calibratedScore &&
      a.calibration?.calibratedScore !== a.calibration?.originalCalculatedScore,
  ).length;
  ...
  const scoreCounters = {
    "1.0 - 2.2": filteredAppraisals.filter((a) => {
      const s =
        a.calibration?.calibratedScore ||
        a.managerAppraisal?.calculatedScore ||
        0;
      return s >= 1.0 && s <= 2.2;
    }).length,
  ...
  // ===== DEPARTMENT AVERAGES =====
  const deptAverages = departments
    .filter((d) => d !== "ALL")
    .map((dept) => {
      const appsInDept = mockAppraisals.filter((a) => {
        const u = mockUsers.find((usr) => usr.id === a.employeeId);
        return u?.department === dept && a.cycleId === mockActiveCycle?.id;
      });

      const rated = appsInDept.filter(
        (a) =>
          a.calibration?.calibratedScore || a.managerAppraisal?.calculatedScore,
      );
      const avg =
          ...
          : parseFloat(
              (
                rated.reduce(
                  (acc, cur) =>
                    acc +
                    (cur.calibration?.calibratedScore ||
                      cur.managerAppraisal?.calculatedScore ||
                      0),
                  0,
                ) / rated.length
              ).toFixed(2),
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-4 md:p-6 font-sans select-text">
      ...
      <section className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4">
        <div>
          <h2 className="text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            <BarChart3 className="text-blue-900" size={24} /> Executive Talent
            Diagnostics
          </h2>
          <p className="text-xs text-slate-500 mt-1 font-medium">
            Analyze consolidated reviews analytics, inspect cohort ratings
            alignment, and audit organizational parity levels globally.
          </p>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-3xs grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="space-y-1 text-xs">
          <label className="font-bold text-slate-705 flex items-center gap-1.5 mb-1.5">
            <MapPin size={14} className="text-blue-900" /> Geographical Site
            Filter
          </label>
          <select
        ...
        <div className="space-y-1 text-xs font-sans">
          <label className="font-bold text-slate-705 flex items-center gap-1.5 mb-1.5">
            <Building size={14} className="text-blue-900" /> Organizational
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="mt-4 space-y-1.5">
            <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${((averageRating || 3.8) / 5) * 100}%` }}
              />
            </div>
            <p className="text-[10px] text-slate-400 font-medium">
              Consolidated live performance trend
            </p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-3xs flex flex-col justify-between min-h-[120px] transition-all">
          <div className="space-y-1">
            ...
            <div className="flex items-baseline gap-1.5">
              ...
              <span className="text-[10px] text-slate-400 font-bold">
                {completedScenarios} of {totalInScope} appraisals
              </span>
            </div>
          </div>
          <div className="mt-4 space-y-1.5">
            ...
            <p className="text-[10px] text-slate-400 font-medium">
              Fully locked and released cycles
            </p>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-3xs flex flex-col justify-between space-y-2 transition-all">
          ...
          <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[10px] font-bold text-amber-600 font-sans">
            ...
            <span className="bg-amber-50 px-1.5 py-0.5 rounded uppercase font-mono text-[9px] border border-amber-200">
              Active
            </span>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-3xs flex flex-col justify-between space-y-2 transition-all">
          ...
          <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[10px] font-bold text-slate-400 font-sans">
            ...
            <span className="bg-slate-50 px-1.5 py-0.5 rounded font-mono text-[9px] border border-slate-200">
              Satisfactory
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        {/* Left Side: Cohort Performance Bell Curve Chart */}
        <section className="lg:col-span-8 bg-white border border-slate-200 rounded-2xl p-6 shadow-3xs flex flex-col justify-between space-y-4">
          <div>
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500 font-mono flex items-center gap-1.5">
              <TrendingUp size={16} className="text-blue-900" /> Bell Curve
              Parity Cohort Distribution
            </h3>
            <p className="text-[10px] text-slate-400 mt-0.5">
              Metrics compared side-by-side with recommended normal-curve
              guidelines.
            </p>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        height: `${count === 0 ? 3 : Math.max(percentageHeight, 15)}%`,
                      }}
                    >
            ...
            <div className="mt-5 border-t border-slate-200 pt-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 text-[10px] text-slate-500 leading-normal font-sans">
              <span>
                * Target Curve parameters:{" "}
                <b>10% High Performer / Specialist</b> (4.3-5.0), and{" "}
                <b>70% Core Consistent Delivery</b> (3.3-4.2).
              </span>
        ...
        <aside className="lg:col-span-4 bg-white border border-slate-200 rounded-2xl p-6 shadow-3xs flex flex-col justify-between space-y-4">
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">
              Averages across direct corporate divisions.
            </p>
          </div>
          ...
          <div className="space-y-4 flex-1 flex flex-col justify-center">
            {deptAverages.map((dept) => {
              ...
              return (
                <div
                  key={dept.name}
                  className="space-y-1.5 font-sans text-xs select-text"
                >
                  ...
                  <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${scorePercent > 0 ? scorePercent : 0}%`,
                      }}
                    />
                  </div>
                  <div className="flex justify-between text-[9px] text-slate-400 font-semibold uppercase tracking-wider">
                    ...
                    <span>
                      Count: {dept.totalCount}{" "}
                      {dept.totalCount === 1 ? "profile" : "profiles"}
                    </span>
                  </div>
                </div>
              );
