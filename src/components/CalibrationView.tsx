...
// ============ COMPONENT ============
export const CalibrationView: React.FC<CalibrationViewProps> = ({ user }) => {
  ...
  // Find appraisals ready for calibration
  const calibratableAppraisals = mockAppraisals.filter((a) => {
    return (
      a.cycleId === mockActiveCycle?.id &&
      (a.currentStage === "Calibration" ||
        a.currentStage === "SignOff" ||
        a.currentStage === "Completed")
    );
  });

  const activeApp =
    mockAppraisals.find((a) => a.id === selectedAppId) ||
    calibratableAppraisals[0];
  const activeEmployee = activeApp
    ? mockUsers.find((u) => u.id === activeApp.employeeId)
    : null;

  // Sync state values with selected appraisal
  useEffect(() => {
    if (activeApp) {
      const originalScore =
        activeApp.calibration?.calibratedScore ||
        activeApp.managerAppraisal?.calculatedScore ||
        3.0;
  ...
  const handleSelectAppraisal = (app: any) => {
  ...
  const handleAction = (isSubmit: boolean) => {
    ...
    if (!changeReason.trim() || changeReason.length < 25) {
      setErrorMsg(
        "Audit Compliance Check: Calibration change reason justification must be at least 25 characters details.",
      );
      return;
    }

    // TODO: Gọi calibrateAppraisal từ DataverseContext
    // calibrateAppraisal(activeApp.id, calibratedValue, changeReason, isSubmit);

    if (isSubmit) {
      setSuccessMsg(
        `Successfully finalized, submitted, and LOCKED performance rating for ${activeEmployee?.name} to score ${calibratedValue.toFixed(1)}/5.0`,
      );
    } else {
      setSuccessMsg(
        `Successfully saved draft calibration details for ${activeEmployee?.name} to score ${calibratedValue.toFixed(1)}/5.0`,
      );
    }
  };
      ...
      <section className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <h2 className="text-xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
            <TrendingUp className="text-blue-900" size={20} /> Corporate
            Calibration Room
          </h2>
          <p className="text-xs text-slate-500 mt-1">
            Conduct multi-site cohort rating alignments, adjust skewed manager
            distributions, and maintain organizational equity.
          </p>
        </div>
      ...
      {!hasAccess ? (
        <div className="bg-amber-50 border border-amber-100 p-6 rounded-xl text-amber-800 space-y-2 max-w-lg mx-auto text-center font-sans">
          ...
          <h3 className="font-bold text-sm">
            Access Denied: Exclusivity Parameters
          </h3>
          <p className="text-xs text-amber-700 leading-relaxed">
            The Calibration Panel is locked to HR Administrators only. Toggle
            your act as persona in the top header to{" "}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 space-y-4">
            <h3 className="text-xs font-bold uppercase tracking-widest text-slate-550 font-mono">
              Cohorts Under Review
            </h3>
            ...
            {calibratableAppraisals.length === 0 ? (
              <div className="p-6 bg-slate-50 border rounded-xl text-center text-slate-400 text-xs font-sans">
                Review submissions has not concluded manager evaluation step. No
                calibrated targets ready.
              </div>
            ) : (
              <div className="space-y-2.5">
                {calibratableAppraisals.map((apr) => {
                  const emp = mockUsers.find((u) => u.id === apr.employeeId);
                  const isSelected = activeApp?.id === apr.id;
                  const currentScore =
                    apr.calibration?.calibratedScore ||
                    apr.managerAppraisal?.calculatedScore ||
                    0;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-right leading-none font-sans">
                        ...
                        <span className="text-sm font-black font-mono text-indigo-905">
                          {currentScore
          ...
          <div className="lg:col-span-7 space-y-6 select-text">
            {activeApp && activeEmployee ? (
              <div className="bg-white border border-slate-200.5 rounded-xl p-6 shadow-xs space-y-5">
                <div className="border-b border-slate-100 pb-3 flex justify-between items-center bg-slate-50/20 px-2 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <h4 className="text-xs font-bold text-slate-805 uppercase font-sans">
                        Calibrate Focus: {activeEmployee.name}
                      </h4>
                      <p className="text-[10px] text-slate-400">
                        Position: {activeEmployee.title} —{" "}
                ...
                <div className="space-y-4 text-xs">
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 grid grid-cols-3 gap-4 text-center font-sans">
                    <div>
                      <span className="block text-[9px] font-bold uppercase text-slate-450 mb-0.5">
                        Manager Score
                      </span>
                      <strong className="text-sm font-black font-mono text-slate-800">
                        {activeApp.managerAppraisal?.calculatedScore?.toFixed(
                          2,
                        ) || "3.0"}
                      </strong>
                    </div>
                    <div>
                      <span className="block text-[9px] font-bold uppercase text-slate-450 mb-0.5">
                        Manager Rating goals
                      </span>
                      <strong className="text-sm font-black font-mono text-slate-800">
                        {activeApp.managerAppraisal?.ratingGoals?.toFixed(1) ||
                          "3.0"}
                      </strong>
                    </div>
                    <div>
                      ...
                      <strong className="text-sm font-black font-mono text-slate-800">
                        {activeApp.managerAppraisal?.ratingCompetencies?.toFixed(
                          1,
                        ) || "3.0"}
                      </strong>
                    </div>
                  ...
                  >
                    <div
                      className={`rounded-xl p-5 space-y-3 shadow-inner ${activeApp.calibration?.isSubmitted ? "bg-slate-800 text-slate-400" : "bg-slate-900 text-white"}`}
                    >
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setCalibratedValue(Number(e.target.value))
                        }
                      ...
                      <p className="text-[10px] text-slate-400">
                        {activeApp.calibration?.isSubmitted
                          ? "Calibration rating has been completed and locked."
                          : "Adjust score values smoothly. Calibrated scores override manager ratings during overall talent calibration calculations."}
                      </p>
                    </div>
                    ...
                    <div className="space-y-1.5 font-sans">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-mono font-bold text-[10px] text-slate-500">
                          {changeReason.trim().length} chars
                        </span>
                      </div>
                      <textarea
                        ...
                        <div className="flex items-start gap-2.5">
                          ...
                          <div>
                            <span className="font-bold block text-slate-800 leading-snug">
                              Calibration Sealed & Locked
                            </span>
                            <span className="text-[10px] text-slate-500 block leading-normal mt-0.5">
                              Approved by{" "}
                              {activeApp.calibration?.calibratedBy ||
                                "Corporate HR"}{" "}
                              on{" "}
                              {activeApp.calibration?.calibratedAt
                                ? new Date(
                                    activeApp.calibration.calibratedAt,
                                  ).toLocaleDateString()
                                : "N/A"}
              ...
              <div className="bg-slate-50 border border-slate-200.5 rounded-xl p-10 text-center font-sans text-slate-400 text-xs">
                ...
                Select an employee from the cohort list to adjust rating
                metrics.
              </div>
            )}
          </div>
