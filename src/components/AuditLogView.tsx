...
export const AuditLogView: React.FC<AuditLogViewProps> = ({ user }) => {
  ...
  const {
    // TODO: Thêm auditLogs vào DataverseContext
    // auditLogs,
    // loadAuditLogs
  } = useDataverse();
  ...
  const mockAuditLogs: AuditLog[] = [
    ...
    },
    {
      ...
      details: "New appraisal cycle created: FY2024 Mid-Year Review",
    },
    {
      id: "3",
      ...
      details: "Approved goals for employee: John Doe",
    },
  ];

  // ===== LOAD AUDIT LOGS =====
  useEffect(() => {
    // TODO: Gọi loadAuditLogs từ DataverseContext
    // loadAuditLogs();
      ...
      <section className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-205 pb-4">
        <div>
          <h2 className="text-xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
            <History className="text-blue-900" size={20} /> Immutable Audit Logs
            Ledger
          </h2>
          <p className="text-xs text-slate-500 mt-1">
            System logs tracking goal creation, rating changes, calibration
            adjustments, sign-offs, and administrative overrides.
          </p>
        </div>
        <div className="flex items-center gap-2 text-xs bg-emerald-50 text-emerald-800 border border-emerald-100 py-1.5 px-3.5 rounded-lg font-bold font-mono">
          <ShieldCheck size={14} className="text-emerald-700 animate-pulse" />{" "}
          LEDGER SEALED (AES-256)
        </div>
      </section>
      ...
      {!isHR ? (
        <div className="bg-amber-50 border border-amber-100 p-6 rounded-xl text-amber-800 space-y-2 max-w-lg mx-auto text-center font-sans">
          ...
          <p className="text-xs text-amber-700 leading-relaxed">
            Audit logs are confidential and only accessible to HR and Senior
            Management.
          </p>
        </div>
      ) : (
                ...
                <table className="w-full text-left font-sans text-xs">
                  ...
                  <tbody className="divide-y divide-slate-150">
                    {filteredLogs.map((log) => {
                      // Generate a custom deterministic signature hash
                      const mockVerificationCode = `SHA-${(
                        log.timestamp + log.action
                      )
                        .split("")
                        .reduce((acc, char) => acc + char.charCodeAt(0), 100)
                        .toString(16)
                        .toUpperCase()}`;
                      ...
                      return (
                        <tr
                          key={log.id}
                          className="hover:bg-slate-50 transition-all text-slate-705 font-medium"
                        >
                          ...
                          <td className="px-5 py-3.5">
                            <span
                              className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded font-sans ${
                                ...
                                log.action.includes("Success")
                                  ...
                                  : log.action.includes("Unlock") ||
                                    ...
                                    : log.action.includes("Calibration") ||
                    ...
                    {filteredLogs.length === 0 && (
                      <tr>
                        ...
                        >
                          No matching audit ledgers matches search descriptor
                          filter.
                        </td>
                      </tr>
                    )}
